        ...
        <div className="absolute top-[15%] left-1/2 -translate-x-1/2 flex gap-4">
          {[0, 1, 2].map((index) => (
            <div key={index} className="relative" style={{ animationDelay: `${index * 0.1}s` }}>
              <img
                src={isLit ? '/assets/generated/candle-lit.dim_64x128.png' : '/assets/generated/candle-out.dim_64x128.png'}
              ...
              {isLit && (
                <div className="absolute -top-2 left-1/2 -translate-x-1/2 w-4 h-6 flame-flicker">
                  <div className="w-full h-full bg-gradient-to-t from-party-flame-base via-party-flame-mid to-party-flame-tip rounded-full blur-sm animate-flame"></div>
                </div>
              )}
            </div>
          ))}
